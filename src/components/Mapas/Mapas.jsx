...
const MapWithMarkers = () => {
    ...
    useEffect(() => {
        if (user?.email) {
            fetch(`https://backendexamen-production-23a8.up.railway.app/examen/${user.email}`)
                .then((response) => {
                    if (!response.ok) {
            ...
            fetch(`https://backendexamen-production-23a8.up.railway.app/examen/visitas/${user.email}`)
                .then((response) => {
                    if (!response.ok) {
    ...
    const handleAddLugar = async (e) => {
        ...
        try {
            const response = await fetch("https://backendexamen-production-23a8.up.railway.app/examen", {
                method: "POST",
                body: formData,
            });
